...
int forrandom=0,incpts=0,lives=10,forbr=1,tempforhs;



void resetall()  //to reset all the points and lives
{
    lives=10;
    incpts=0;
    forbr=0;
}
char* convert(int x) // to convert from integer to string
{
    char buf[10];
    itoa(x,buf,10);
    return buf;
...
SDL_Surface* load_image(const char* c,Uint32 colorkey=0) // to set colorkey to remove background color
{
...
int showmenu(SDL_Surface* screen, TTF_Font* font, char *his,int points) // shows menu
{
        Uint32 time;
        int x, y;
        const int NUMMENU = 6;
        const char* labels[NUMMENU] = {"CONTINUE","Exit","HIGH SCORE:",his,"YOUR SCORE",convert(points)};
        SDL_Surface* menus[NUMMENU];
        bool selected[NUMMENU] = {0,0,0,0,0,0};
        SDL_Color color[2] = {{255,255,255},{255,0,0}};
    //menu surface loading
        ...
        pos[4].x = screen->clip_rect.w/2 - menus[0]->clip_rect.w/2;
        pos[4].y = screen->clip_rect.h/2 + menus[0]->clip_rect.h+100;
        pos[5].x = 500;
        pos[5].y = screen->clip_rect.h/2 + menus[0]->clip_rect.h+100;
        //filling background with black color
        SDL_FillRect(screen,&screen->clip_rect,SDL_MapRGB(screen->format,0x00,0x00,0x00));
        //event handling within menu
        SDL_Event event;
        while(1)
        {
                time = SDL_GetTicks();
...
public:
    ...
    int collision(SDL_Rect* rec1,SDL_Rect* rec2)//checks collision between ball and paddle and also with borders of window
    {
    ...
    void move_ball(int k) //changes direction of ball
    {
        if(k==1)
        {
            yvel=-yvel;
    ...
    void ball_reset(int q,int w) //if out then lives are decreased
    {
        rectball.x=q;
        rectball.y=w;
        xvel=-xvel;
...
public:
    ...
    void brick_random()//sets random position of obstacle and also avoids overlapping of bricks
    {
        int abcd=0;
        while(1)
        {
    ...
    int col_brick(SDL_Rect* rec1,SDL_Rect* rec2)//collision detection
    {

      if(rec1->x+rec1->w>=rec2->x && rec1->x<=rec2->x+rec2->w && rec1->y+rec1->h>=rec2->y && rec1->y+rec1->h<=rec2->y+4)
            return 1;
      if(rec1->x+rec1->w>=rec2->x && rec1->x<=rec2->x+rec2->w && rec1->y>=rec2->y+rec2->h-4 && rec1->y<=rec2->y+rec2->h)
            return 2;
      if(rec1->y+rec1->h>=rec2->y && rec1->y<=rec2->y+rec2->h && rec1->x+rec1->w>=rec2->x && rec1->x+rec1->w<=rec2->x+4)
         return 3;
      if(rec1->y+rec1->h>=rec2->y && rec1->y<=rec2->y+rec2->h && rec1->x<=rec2->x+rec2->w && rec1->x>=rec2->x+rec2->w-4)
        return 4;
        else
            return 5;
    }
  SDL_Rect* getrect()//returns coordinates of bricks
  {
      return &rectbr;
  }
...
public:
  ...
  int settarget(SDL_Rect* r1,SDL_Rect* r2)//to ensure no overlapping between obstacle brick and target brick
      {
 ...
 void show_brick()
    ...
    int col_brick(SDL_Rect* rec1,SDL_Rect* rec2)//collision detection between ball and target brick
    {
...
void incopts()// to increase point if target brick is hit
{
    incpts++;
    Mix_PlayChannel(-1,eff1,0);
}

int whatisscore()//returns value of present score
{
    return incpts;
}
void breaking_ball()//randomly sets target brick position
{
...
int checklives(int x)//lives checking
{
    if(x==0)
        {
...
//rectangles for various images
    ...
    bgmus=Mix_LoadMUS("84232-the-scientist-coldplay--1421371608.wav");
    tring=Mix_LoadWAV("smb_breakblock.wav");
    ...
    brick2 bri2(load_image("brick2.bmp",SDL_MapRGB(screen->format,0xff,0xff,0xff)),Mix_LoadWAV("smb_coin.wav"),50+rand()%200,150,30,30);
    brick1 brs[6];
    for(int coun=0;coun<2;coun++)
         brs[coun].getinfo(load_image("brick1.bmp",SDL_MapRGB(screen->format,0xff,0xff,0xff)),150+100*coun,100,30,30);
    for(int coun=2;coun<4;coun++)
         brs[coun].getinfo(load_image("brick1.bmp",SDL_MapRGB(screen->format,0xff,0xff,0xff)),50+100*coun,200,30,30);
    for(int coun=4;coun<6;coun++)
         brs[coun].getinfo(load_image("brick1.bmp",SDL_MapRGB(screen->format,0xff,0xff,0xff)),50+100*coun,300,30,30);
    ...
    while(running)
    {
        ...
        ifstream infile;           //hiscore from file is assigned to tempforhs
        infile.open("hiscore.txt");
        infile>>tempforhs;
        infile.close();
                                                  if(bri2.whatisscore()>tempforhs)//comparision of present score and hiscore
                                                    {
                                                        ofstream outfile;
                                                        outfile.open("hiscore.txt");
                                                        outfile<<bri2.whatisscore()<<endl;
                                                        outfile.close();
                                                    }
        ...
        ifstream infile1;
        infile1.open("hiscore.txt");
        infile1>>tempforhs;
        infile1.close();
              int i = showmenu(screen, fnt, convert(tempforhs),incpts);
                  if(i==1)
                    running = false;
              resetall();
            }
